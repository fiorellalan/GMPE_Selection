#!/usr/bin/env python3
"""
gmpe_selection_gui_linux.py —  Linux‑compatible version.

Same as gmpe_selection_gui.py but adapted for Linux:
  • Dark mode via gsettings/kreadconfig5
  • Font → DejaVu Sans (fallback if Helvetica unavailable)
  • Mouse wheel → Button-4/Button-5 events

Usage:
  python src/gmpe_selection_gui_linux.py
  python src/gmpe_selection_gui_linux.py --catalogue gmpe_catalogue.csv
"""
...
def _detect_dark_mode():
    """Return True if the OS is in dark‑appearance mode (Linux + macOS)."""
    ...
    try:
        if _sys_name == "Darwin":
            ...
            # GNOME fallback: check gtk-theme
            r = subprocess.run(["gsettings", "get", "org.gnome.desktop.interface",
                                "gtk-theme"], capture_output=True, text=True, timeout=2)
            if r.returncode == 0:
                return "dark" in r.stdout.strip().lower()
            # KDE Plasma
            r = subprocess.run(["kreadconfig5", "--group", "General", "--key",
                                "ColorScheme", "~/.config/kdeglobals"],
                               capture_output=True, text=True, timeout=2)
            if r.returncode == 0:
                return "dark" in r.stdout.strip().lower()
    except Exception:
...
def _show_country_keyword_dialog(parent, catalogue, display_rows, filters):
    ...
    # ── Mousewheel / trackpad scrolling (Linux: Button-4/Button-5, others: MouseWheel) ──
...
# ── Mousewheel / trackpad (Linux: Button-4/Button-5, others: MouseWheel) ──
    def _on_mousewheel(event):
        if not canvas.winfo_exists():
            return
        if _pltfrm.system() == "Linux" and not hasattr(event, 'delta'):
...
class GMPESelectionGUI:
    ...
    def __init__(self, catalogue_path):
        ...
        # Adapt window size to screen (90% of screen, max 1400×850)
        _sw = self.root.winfo_screenwidth()
        _sh = self.root.winfo_screenheight()
        _w = min(1400, int(_sw * 0.92))
        _h = min(850, int(_sh * 0.88))
    ...
    def _build_filter_panel(self, parent):
        ...
        # Bind mousewheel / trackpad (Linux: Button-4/Button-5, others: MouseWheel)
        def _on_mousewheel(event):
            if not canvas.winfo_exists():
                return
            if _pltfrm.system() == "Linux" and not hasattr(event, 'delta'):
...
'''
                ...
                oq_python = os.path.expanduser("~/openquake/bin/python")
                proc = subprocess.run(
